...
public class BookServiceImpl implements BookService {
    ...
    public List<BookDTO> getAllBooks() {
        ...
        return bookList.stream().map(bookObj -> objectMapper.convertValue(bookObj, BookDTO.class)).collect(Collectors.toList());
    }

    @Override
    public BookDTO updateBook(BookDTO bookDTO) {
        Book existing = bookRepository.findById(bookDTO.getId()).orElseThrow(() -> new ResourceNotFoundException("Book not found with id " + bookDTO));
        Book book = objectMapper.convertValue(bookDTO, Book.class);
